#!/usr/bin/env python3
# Genera la copertina SVG del manuale, aderente al banner ufficiale.
...
OUT  = ROOT / "branding" / "manuale-copertina-v1.11.5.svg"
...
while x < W-6:
    f = x/(W-12)
    env = 0.45 + 0.55*math.sin(math.pi*min(max(f,0),1))   # più alte al centro
...
svg = f'''<svg width="{W}" height="{H}" viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Copertina del Manuale Utente di Runtime Live Machine Pro, Seconda Edizione, versione 1.11.5">
  <defs>
  ...
  <g>
    ...
    <text x="{CX+6:.0f}" y="482" text-anchor="middle" font-family="Consolas, Menlo, monospace" font-size="14" font-weight="700" letter-spacing="2" fill="#c7cede">v 1.11.5</text>
  </g>
  ...
  <text x="{CX}" y="612" text-anchor="middle" font-family="Segoe UI, Helvetica, Arial, sans-serif" font-size="13" font-weight="700" letter-spacing="6" fill="#00c8d4">MANUALE UTENTE</text>
  <text x="{CX}" y="640" text-anchor="middle" font-family="Segoe UI, Helvetica, Arial, sans-serif" font-size="15" font-weight="500" letter-spacing="1" fill="#c7cede">Seconda Edizione · Versione 1.11.5 · Italiano</text>
...
print("OK", OUT, f"{OUT.stat().st_size//1024} KB", f"{len(bars)} barre")
