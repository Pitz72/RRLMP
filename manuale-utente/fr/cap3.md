...
Runtime Live Machine Pro n'utilise pas de menus complexes "Fichier > Importer". Il est conçu pour travailler directement avec les dossiers de votre ordinateur.
...
*   **Formats Supportés** : Grâce au moteur natif, RRLMP supporte presque tous les formats audio standard : **MP3, WAV, AAC (m4a), OGG, FLAC**.
...
> **Note** : Le logiciel mémorise le "chemin" du fichier (ex. C:\Musique\Song.mp3). Si vous déplacez ou renommez le fichier original sur votre ordinateur, RRLMP ne pourra plus le trouver (la carte deviendra rouge/inactive). Pour éviter ce problème si vous changez de PC, utilisez la fonction "Export Package" (voir Chap. 7).
...
Pendant une émission, les besoins changent. RRLMP vous permet de réorganiser la grille à la volée.
