...
Самый частый случай применения — выравнивание уровней: если у вас голосовая запись, сделанная тихо (например, сообщение из WhatsApp или запись телефонного разговора), её можно поднять выше 100%, приблизив к громкости остальных дорожек. И наоборот, особенно «горячий» клип можно приглушить, не трогая Master Volume.
...
Кнопка со значком **волшебной палочки** запускает автоматическое обнаружение тишины через FFmpeg. Порог не фиксирован: программа сначала оценивает средний уровень файла и выставляет порог тишины примерно на 25 dB ниже этого уровня (в пределах безопасного диапазона от −55 до −20 dB; при отсутствии оценки откатывается к −40 dB). Trim Start и Trim End устанавливаются автоматически, убирая начальную тишину и немые концовки без ручного вмешательства.

Эта функция особенно полезна для необработанных голосовых записей: телефонных звонков, аудиосообщений, интервью, записанных на мобильные устройства. Применение Auto-Trim ко всей колонке «Голос» перед эфиром занимает меньше минуты и повышает чистоту переходов.
...
Рядом с Auto-Trim функция **Smart Cues** автоматически предлагает маркеры Intro и Outro. Используя более агрессивный порог, она находит точку, где звук достигает полной энергии (Intro), и точку, где начинается финальное затухание (Outro), расставляя два маркера без необходимости искать их на слух.
...
Привязки клипов сохраняются в файле проекта: при переносе проекта на другой компьютер с тем же MIDI-контроллером привязки заработают без перенастройки.
