...
Vedi la sezione 2.3: clic destro sull'applicazione → #emph[Apri], oppure
sblocco da #emph[Impostazioni di Sistema → Privacy e sicurezza].
...
applicazione principale: RLMP è un software desktop per Windows e Linux,
compilabile anche su macOS. Un tablet o un telefono possono però fungere
da #strong[telecomando] via browser, tramite il Controllo Remoto
(Capitolo 11).

#strong[I file `.lmp` delle versioni precedenti sono compatibili con la
1.15.33?] Sì. Aprendo un progetto creato con una versione precedente,
...
l'installazione è automatica dalla finestra di aggiornamento; su Linux
`.deb` viene aperto il browser sulla pagina di download; su macOS si
ricompila dal sorgente. Tutti i dettagli nel Capitolo 12.
